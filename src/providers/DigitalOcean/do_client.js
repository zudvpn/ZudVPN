...
class Client {
    ...
    async createSSHKey(name, publicKey) {
        let response = await this.makeRequest('POST', 'https://api.digitalocean.com/v2/account/keys', JSON.stringify({
            name,
    ...
    async createDroplet(sshKeys, name, region, size, data, image, tags) {
        let response = await this.makeRequest('POST', 'https://api.digitalocean.com/v2/droplets', JSON.stringify({
            name,
            region,
            size,
            image,
    ...
    async addDropletToFirewall(firewallId, dropletId) {
        await this.makeRequest('POST', `https://api.digitalocean.com/v2/firewalls/${firewallId}/droplets`, JSON.stringify({
            droplet_ids: [
                dropletId
            ]
        }))
    ...
    async createFirewall(name, dropletId) {
        let response = await this.makeRequest('POST', 'https://api.digitalocean.com/v2/firewalls', JSON.stringify({
            name,
            inbound_rules: [
                {
                    protocol: "icmp",
