...
class Deploy {
    ...
    async run() {
        ...
        if (!droplet) {
            ...
            let userData = GenerateUserData(sshKeyPair.authorizedKey, this.token)
    ...
    async waitForVPNService(sshClient) {
        ...
        do {
            console.log('Cheking VPN service, attemt: ', 10 - countWaitingForVPN)
            this.logger('Cheking VPN service, attemt: ', 10 - countWaitingForVPN)
            countWaitingForVPN--
            try {
    ...
    async waitForSSHConnection(sshClient) {
        ...
        do {
            ...
            this.logger('Waiting for SSH, attempt:', 10 - trialLeft)
            trialLeft--
            try {
